...
public class ClassifierTree implements Drawable, Serializable,
  ...
  public void buildTree(Instances data, boolean keepData) throws Exception {
    ...
    // set labels
    m_label = ((Instances)data).classAttribute().value(m_localModel.distribution().maxClass());
    // set node attribute
    m_attribute = m_localModel.leftSide(m_train);
    //m_attribute = "X";
  ...
  public String toString() {
    ...
    try {
      ...
      if (m_isLeaf) {
    	//text.append("Instances: " + m_instances + " Influence: " + m_influence + " % " + "Class: " + m_class + " *");
        text.append(": ");
        text.append(m_localModel.dumpLabel(0, m_train));
      } else {
        dumpTree(0, text);
	  ...
	  if((this.isLeaf()))
	  {
		  ...
		  while(current.m_parent!=null) {
			  ...
			  if(!m_train.attribute(current.m_parent.getAttributeName()).isNominal()) {
				  ...
				  double perbag[] = current.m_parent.m_localModel.m_distribution.m_perBag;
				  int bestindex = -1;
				  double bestscore = -1;
  ...
  public String leaveClass() {return ((Instances)m_train).classAttribute().value(m_localModel.m_distribution.maxClass());}
  public double leaveInstances() {return m_localModel.m_distribution.total();}
  public double leaveCorrectInstances() {return m_localModel.m_distribution.numCorrect();}
  public double splitPoint() {return ((C45Split)m_localModel).m_splitPoint;}
  public ClassifierTree m_parent = null;
  public int m_son_index = -1;
  public ArrayList<ClassifierTree> m_rootPath = new ArrayList<ClassifierTree>();
  public String m_label = "";
  public String m_attribute = "";
  public String sonSide = "";
